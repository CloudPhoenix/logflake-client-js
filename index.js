...
let LogFlakeServer = process.env.LOGFLAKESERVER || 'https://app-test.logflake.io'
let LogFlakeAppId = null
let LogFlakeHostname = null

const Post = async (uri, bodyObject) => {
  if (LogFlakeAppId === null) throw new Error('App ID must not be empty. Call Setup first.')
  const retries = 3
...
const Setup = (appid = null, server = null, hostname = null) => {
  if (appid === null || appid.length === 0) throw new Error('App ID must not be empty')
  LogFlakeAppId = appid
  if (server !== null) LogFlakeServer = server
  LogFlakeHostname = hostname || os.hostname() || null
}

const SendLog = (level, content = null, correlation = null, params = {}) =>
  Post('logs', {
    correlation,
    params,
...
const SendPerformance = (label = null, duration = null) => {
  if (label === null || duration === null) throw new Error('Label and duration must not be empty')
...
module.exports = {
  Setup, LogLevels, SendLog, SendException, SendPerformance, MeasurePerformance
}
